<x-app-layout>
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- 
                $incidentData = Data LENGKAP dari API (untuk relasi seperti nama user, site, komentar)
                $incident     = Data LOKAL dari database (untuk status terbaru)
            --}}
            ...
            @if($incidentData)
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6 sm:p-8 border-b border-gray-200">
                        ...
                        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start">
                            <h1 class="text-3xl font-bold text-gray-900 leading-tight mb-2 sm:mb-0">
                                {{-- Ambil Judul dari data API --}}
                                {{ $incidentData['title'] }}
                            </h1>
                            
                            {{-- Badge Status Dinamis --}}
                            @php
                                // !! PENTING: Ambil status dari $incident (LOKAL)
                                // karena ini yang paling update via queue
                        ...
                        <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4 text-sm text-gray-600">
                            <div class="flex items-center space-x-2">
                                ...
                                <span>Dilaporkan oleh: <strong class="font-semibold text-gray-800">{{ $incidentData['user']['name'] ?? 'N/A' }}</strong></span>
                            </div>
                            <div class="flex items-center space-x-2">
                    ...
                    {{-- Bagian Aset dan Komentar (dari API) --}}
            ...
            @else
                {{-- Ini adalah blok 'else' yang akan tampil jika $incidentData = null (API gagal) --}}
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-center text-gray-600">
                        <p>Gagal memuat detail laporan dari server ticket. Silakan coba lagi nanti.</p>
                    </div>
                </div>
            @endif
        </div>
